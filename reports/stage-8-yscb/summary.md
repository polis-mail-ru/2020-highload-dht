...
При отработке нагрузки А использовались 1 000 000 операций половина из которых - чтение, другая половина - запись.
Распределение - равномерное.
Количество ключей - 1 000 000.

Запись прошла успешно для 499374 запросов.
95% всех спросов на запись проходит за 1243 микросекунд или меньше.

Чтение прошло успешно для 131740 запросов.
Чтение прошло НЕ успешно для 368886 запросов. Это связано с тем, что база изначально пустая. 
95% всех спросов на чтение проходит за 1200 микросекунд или меньше.

------------------------------------------------------------------------------------------------------------------

При отработке нагрузки B использовались 1 000 000 операций 95% из которых - чтение, 5% - запись.
Распределение - равномерное.
Количество ключей - 1 000 000.

Запись прошла успешно для 50285 запросов.
95% всех спросов на запись проходит за 928 микросекунд или меньше.

Чтение прошло успешно для 420928 запросов.
Чтение прошло НЕ успешно для 528787 запросов. 
После работы первой нагрузки в базе уже есть данные, но их гораздо меньше чем операций 
чтения в нагрузке В.    
95% всех спросов на чтение проходит за 830 микросекунд или меньше.

------------------------------------------------------------------------------------------------------------------

При отработке нагрузки С использовались 1 000 000 операций половина из которых всё - чтение.
Распределение - zipfian (Сначала много запросов с постепенным угасанием нагрузки впоследствии).
Количество ключей - 1 000 000.

Чтение прошло успешно для 829790 запросов.
Чтение прошло НЕ успешно для 170210 запросов. 
Постепенно база заполняется данными, что выражается во все большем количестве успешных операций чтения.
95% всех спросов на чтение проходит за 820 микросекунд или меньше.

------------------------------------------------------------------------------------------------------------------

При отработке нагрузки D использовались 1 000 000 операций 95% из которых - чтение, 5% - запись.
Распределение - hotspot (внезапные скачки нагрузки).
Количество ключей - 1 000 000.

Запись прошла успешно для 50078 запросов.
95% всех спросов на запись проходит за 917 микросекунд или меньше.

Чтение прошло успешно для 566046 запросов.
Чтение прошло НЕ успешно для 383876 запросов.  
95% всех спросов на чтение проходит за 819 микросекунд или меньше.
