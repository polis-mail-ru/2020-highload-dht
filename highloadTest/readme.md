...
В данной конфигурацией за 40 сек было выполнено 399829 запросов. В среднем по 25800 req/s и с задержкой в 848us.
99.9% запросов были выполнены с задержкой меньше 7.45ms, достойно.

###CPU 

На данных графиках появился процесс 'proxy'(19%), который переадресовывает запрос на соответсвующую ноду кластера.
...
Аналогично графикам CPU можно также заметить новые графы. Но отношения их ресурсопотребление сильно отличается 19/1% и 23/17%.
...
Потокобезопасность обеспечивается контролем доступа к общим данным экземпляром ReentrantLock и блокировками на чтение и запись данных.
...
В данной конфигурацией за 40 сек было выполнено 399822 запроса. В среднем по 25700 req/s и с задержкой в 843us.
99.9% запросов были выполнены с задержкой меньше 11.84ms.
    
###CPU 

Аналогично результатам PUT запроса на графиках появились процессы навигации запроса к нужной ноде.
...
При увеличении размера кластера таким методом нужно будет платить повышенными временными затратами на обработку и переадресацию запросов к нужной ноде и большим потребление RAM. 