...
На очередном этапе разработки highload-приложения получены и исследованы оценки быстродействия сервера с поддержкой шардирования. Для выполнения распределённых операций единичный сервер, реализовывавший логику HTTP-запросов на предыдущих этапах, заменён на кластер, состоящий из 3 узлов. В рамках текущего анализа предлагается сравнение результатов нагрузочного тестирования с <em>wrk2</em>, а также профилей, полученных в процессе симулирования запросов в нераспределённой (<em>single node</em>) и кластерной (<em>multi-node</em>) конфигурациях сервера.<br/> Тесты с применением <em>wrk2</em> проведены в виде последовательных сеансов подачи PUT- и GET-запросов в течение фиксированного периода (7 минут) обмена данными с пулом клиентов (число клиентов (соединений с сервером) в каждой конфигурации установлено равным 64, количество параллельных потоков - 2 по числу ядер процессора). Интенсивность формирования / отправки запросов каждого вида (<em>Rate</em>) в конфигурации с единственным узлом задана равной 15000 запросов/с, в кластере - 10000 запросов/с исходя из предварительных выводов о нагрузке, при которой фактическая производительность генератора запросов стабилизируется на максимально близких относительно уровня <em>Rate</em> значениях.
...
![async_put_cpu](resources/flamegraphs/put_cpu_single.svg)
<p align="center">Рис.1. Выделение ресурса CPU при симулировании PUT-запросов (single-node)</p>

![async_put_cpu](resources/flamegraphs/put_cpu_multi.svg)
...
![async_put_alloc](resources/flamegraphs/put_alloc_single.svg)
<p align="center">Рис.3. Выделение ресурса RAM при симулировании PUT-запросов (single-node)</p>

![async_put_alloc](resources/flamegraphs/put_alloc_multi.svg)
...
![async_put_lock](resources/flamegraphs/put_lock_single.svg)
<p align="center">Рис.5. Профиль lock/monitor при симулировании PUT-запросов (single-node)</p>

![async_put_lock](resources/flamegraphs/put_lock_multi.svg)
...
![async_get_cpu](resources/flamegraphs/get_cpu_single.svg)
<p align="center">Рис.7. Выделение ресурса CPU при симулировании GET-запросов (single node)</p>

![async_get_cpu](resources/flamegraphs/get_cpu_multi.svg)
...
![async_get_alloc](resources/flamegraphs/get_alloc_single.svg)
<p align="center">Рис.9. Выделение ресурса RAM при симулировании GET-запросов (single node)</p>

![async_get_alloc](resources/flamegraphs/get_alloc_multi.svg)
...
![async_get_lock](resources/flamegraphs/get_lock_single.svg)
<p align="center">Рис.11. Профиль lock/monitor при симулировании GET-запросов (single node)</p>

![async_get_lock](resources/flamegraphs/get_lock_multi.svg)
