...
На очередном этапе разработки highload-приложения получены и исследованы оценки быстродействия сервера с поддержкой шардирования. Для обеспечения распределённой обработки запросов единичный сервер (<em>single node</em>), реализовываший логику обслуживания на предыдущих этапах, заменён на кластер, состоящий из 3 узлов (<em>multi-node</em>). В рамках текущего анализа предлагается сравнение выходных данных стресс-тестов, а также профилей, полученных за продшествующий и актуальный отчётные периоды.<br/> Нагрузочные испытания с применением <em>wrk2</em> проведены в виде серий PUT- и GET-запросов фиксированной продолжительности (7 минут) в режиме симулирования обмена данными с пулом клиентов (число клиентов (соединений с сервером) установлено равным 64, количество параллельных потоков - 2 по числу ядер ЦПУ). Интенсивность генерации / отправки запросов каждого вида (<em>Rate</em>) задана равной 15000 запросов/с исходя из предварительных выводов о нагрузке, при которой генератором запросов поддерживается максимальная приближенность уровню <em>Rate</em>.
