...
Контроль асинхронного параллелизма при выполнении запросов на чтение реализуется с использованием того же механизма взаимоисключений, что обеспечивает безопасный доступ к разделяемым данным в режиме вставки записей (комбинация <em>ReentrantLock</em> и <em>AbstracyQueueSynchronizer</em>).
