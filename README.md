...
Контроль асинхронного выполнения потоков при выполнении GET-запросов реализуется с использованием того же механизма взаимоисключений, который гарантирует безопасность критической секции в очереди с PUT-запросами (комбинация экземпляров <em>ReentrantLock</em> и <em>AbstractQueueSynchronizer</em>).
