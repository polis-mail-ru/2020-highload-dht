...
На очередном этапе разработки highload-приложения получены и исследованы оценки быстродействия сервера с поддержкой шардирования. Для обеспечения распределённой обработки запросов единичный сервер (<em>single node</em>), реализовываший логику обслуживания на предыдущих этапах, заменён на кластер, состоящий из 3 узлов (<em>multi-node</em>). В рамках текущего анализа предлагается сравнение выходных данных стресс-тестов, а также профилей, полученных в процессе симулирования запросов в одиночной и кластерной конфигурациях сервера.<br/> Нагрузочные испытания с применением <em>wrk2</em> проведены в виде серий PUT- и GET-запросов фиксированной продолжительности (7 минут) в режиме симулирования обмена данными с пулом клиентов (число клиентов (соединений с сервером) в каждой конфигурации установлено равным 64, количество параллельных потоков - 2 по числу ядер ЦПУ). Интенсивность формирования / отправки запросов каждого вида (<em>Rate</em>) задана равной 15000 запросов/с на основе предварительных выводов о нагрузке, при которой фактическая производительность генератора запросов стабилизируется на максимально близких относительно уровня <em>Rate</em> результатах.

**Команды <em>wrk2</em>**<br/>

<p></p>

<ins><em>wrk2</em> | PUT</ins>
```
wrk -t2 -c64 -d7m -s src/profiling/wrk_scripts/put.lua -R15000 --latency http://127.0.0.1:8080
```

<ins><em>wrk2</em> | GET</ins>
```
...
Transfer/sec:      0.96MB
```
<ins><em>wrk2</em> outputs / **multi-node** server</ins>  
```
...
<ins><em>wrk2</em> outputs / **single node** server</ins>  
```
